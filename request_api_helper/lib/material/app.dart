...
/// [RequestApiHelperApp] uses this [TextStyle] as its [DefaultTextStyle] to encourage
...
/// Describes which theme will be used by [RequestApiHelperApp].
enum ThemeMode {
...
/// [GridPaper].
///
/// The [RequestApiHelperApp] configures the top-level [Navigator] to search for routes
...
///
/// {@tool snippet}
/// This example shows how to create a [RequestApiHelperApp] that disables the "debug"
/// banner with a [home] route that will be displayed when the app is launched.
///
/// ![The RequestApiHelperApp displays a Scaffold ](https://flutter.github.io/assets-for-api-docs/assets/material/basic_material_app.png)
///
/// ```dart
/// RequestApiHelperApp(
///   home: Scaffold(
...
///
/// {@tool snippet}
/// This example shows how to create a [RequestApiHelperApp] that uses the [routes]
...
///
/// {@tool snippet}
/// This example shows how to create a [RequestApiHelperApp] that defines a [theme] that
/// will be used for material widgets in the app.
///
/// ![The RequestApiHelperApp displays a Scaffold with a dark background and a blue / grey AppBar at the top](https://flutter.github.io/assets-for-api-docs/assets/material/theme_material_app.png)
///
/// ```dart
/// RequestApiHelperApp(
///   theme: ThemeData(
...
///   home: Scaffold(
///     appBar: AppBar(
///       title: const Text('RequestApiHelperApp Theme'),
///     ),
///   ),
/// )
/// ```
...
class RequestApiHelperApp extends StatefulWidget {
  /// Creates a RequestApiHelperApp.
  ///
  ...
  /// Creates a [RequestApiHelperApp] that uses the [Router] instead of a [Navigator].
  const RequestApiHelperApp.router({
    Key? key,
    this.scaffoldMessengerKey,
  ...
  ///    and [darkTheme] in [RequestApiHelperApp].
  ...
  /// ```
  ///
  /// Constructing a [RequestApiHelperApp] with a `FooLocalizationsDelegate` overrides
  ...
  /// {@template flutter.material.RequestApiHelperApp.scrollBehavior}
  ...
  @override
  State<RequestApiHelperApp> createState() => _RequestApiHelperAppState();

  /// The [HeroController] used for Material page transitions.
  ///
  /// Used by the [RequestApiHelperApp].
...
/// Describes how [Scrollable] widgets behave for [RequestApiHelperApp]s.
///
...
class MaterialScrollBehavior extends ScrollBehavior {
  ...
  Widget buildOverscrollIndicator(BuildContext context, Widget child, ScrollableDetails details) {
    // When modifying this function, consider modifying the implementation in
    // the base class as well.
    final AndroidOverscrollIndicator indicator = _androidOverscrollIndicator ?? Theme.of(context).androidOverscrollIndicator ?? androidOverscrollIndicator;
    switch (getPlatform(context)) {
      case TargetPlatform.iOS:
      case TargetPlatform.linux:
      case TargetPlatform.macOS:
...
class _RequestApiHelperAppState extends State<RequestApiHelperApp> {
  ...
  void initState() {
    super.initState();
    _heroController = RequestApiHelperApp.createMaterialHeroController();
  }
  ...
  Widget _buildWidgetApp(BuildContext context) {
    ...
    if (((widget.navigatorKey ?? RequestApiHelper.baseData?.navigatorKey) == null)) {
      throw ErrorDescription('\n\nFix This Error\n\nRequestApiHelper.init() Required before runApp()\n\nOR\n\nyou can add NavigatorKey Manually\n');
    }
    if (_usesRouter) {
    ...
    final List<NavigatorObserver> observer = [RequestApiHelperObserver()];
    if (widget.navigatorObservers != null) {
      for (final i in widget.navigatorObservers ?? []) {
        if (!(i is RequestApiHelperObserver)) {
          observer.add(i);
        }
      }
    }
    ...
    return WidgetsApp(
      key: GlobalObjectKey(this),
      navigatorKey: widget.navigatorKey ?? RequestApiHelper.baseData?.navigatorKey,
